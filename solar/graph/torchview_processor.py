...
class TorchviewProcessor:
    ...
    def _extract_from_hierarchy(
        ...
        for module_node, children_list in node_hierarchy.items():
            try:
                ...
                full_module_id = (
                    f"{parent_name}.{module_name}" if parent_name else module_name
                )

                # Extract the ModuleNode info
                node_info = self._extract_node_info(
                    module_node, full_module_id, original_model
                )
    ...
    def _parse_tensor_shapes_from_attributes(
        ...
        for match in re.finditer(r"Tensor\(shape=\(([^)]*)\)", attributes):
            dims_str = match.group(1).strip()
            if not dims_str:
                continue
            try:
                shape = [int(d.strip()) for d in dims_str.split(",") if d.strip()]
    ...
    def _infer_parameter_name(
        self, op_name: str, arg_index: int, shape: TensorShape
    ) -> str:
