...
软件栈可验证的各精度；固定 official corpus 的 9 个兼容 workload 全部正式
attested，NVFP4 保留为明确不兼容，五个覆盖轴门禁通过。MI300X、MI325X、MI350X
等其他硬件按项目决策延期，因此不能据此声明“P1.6 对全部 AMD 型号已关闭”。

这里的“已关闭”指评分链对上述声明支持子集是 fail-closed 且证据可重放，不表示
已覆盖论文全部 workload 或全部 AMD 架构。未覆盖的算子或无法安全组合的
multi-einsum 图仍不会降级为近似正式分数。
...
组合。普通物化 reshape 和需要 multi-einsum solver 的内部 operand 不会被猜测；若
没有可组合层，正式分析直接失败。
...
记录 `resource_work`。低精度 scale、cast、dequantization 和 FP32 accumulation 都显式
进入计数；一元素退化 reduction 只作为命名 exemption，不伪造计算量。
...
- 10/10 workload 有终态兼容性证据且 `fallbacks_used: []`；
- 9/9 RX 9060 XT 兼容项都有可回放 source-to-SOL attestation，并通过独立 golden；
- official OCP FP8 block-scale workload 正式进入 `fp8->fp32` MFMA 与 scale/cast 成本；
- NVFP4 是唯一明确不兼容项，不做 dtype/shape/device/backend 替换；
- operation（Attention/Norm/MoE/SSM/Conv）、dtype（FP32/BF16/FP8）、forward/backward、
  static/两档 dynamic shape、random/random-scalar/custom input 的必需桶均至少有一项
  formal artifact；空桶使 `formal_coverage_complete` 失败；
...
这关闭的是代表性 AMD corpus 门禁，不是“论文全部题目已覆盖”的声明。
...
P0 与 RX 9060 XT 范围的 P1.5/P1.6/P1.7 已完成。后续工作不应回退当前门禁：

1. 为 MI300X/MI325X/MI350X 等型号分别建立 architecture/precision profile、软件
   支持矩阵与锁频证据；
2. 在保持五轴 fail-closed requirement 的前提下扩大 representative corpus，而不是
   仅提高分母；
3. 评估 P2 的 value-aware 模型，并继续拒绝无安全组合证明的 multi-einsum 近似。
