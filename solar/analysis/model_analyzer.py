...
class ModelAnalyzer:
    ...
    def _register_builtin_handlers(self) -> None:
        ...
        handlers = {
            # Matrix operations
            "matmul": {"einsum": self.einsum_analyzer.generate_matmul_einsum},
            "linear": {"einsum": self.einsum_analyzer.generate_linear_einsum},
            # Convolution operations
            "conv1d": {"einsum": self.einsum_analyzer.generate_conv1d_einsum},
            "conv2d": {"einsum": self.einsum_analyzer.generate_conv2d_einsum},
            "conv3d": {"einsum": self.einsum_analyzer.generate_conv3d_einsum},
            # Elementwise operations
            "relu": {
                "einsum": lambda shape, **kw: self.einsum_analyzer.generate_elementwise_einsum(
                    shape, "relu"
                )
            },
            "sigmoid": {
            ...
            "sum": {
                "einsum": lambda shape, dims=None, **kw: self.einsum_analyzer.generate_reduction_einsum(
                    shape, "sum", dims
                )
            },
            "mean": {
    ...
    def _convert_torchview_to_model_info(
        ...
        """
        model_info = {"model_name": "torchview_model", "layers": {}}
    ...
    def _build_graph_from_model_info(self, model_info: Dict[str, Any]) -> nx.DiGraph:
    ...
    def expand_complex_operations_in_graph(self, graph: nx.DiGraph) -> nx.DiGraph:
    ...
    def _handle_unknown_node_type(
        ...
        try:
            ...
            code, metadata = self.agent.generate_conversion_code(node_type, node_data)
        ...
        if handler and handler.can_generate_einsum():
            try:
                ...
                if einsum_op:
                    ...
                    analysis["memory_elements"] = self.einsum_analyzer.get_memory_cost(
                        shapes
                    )
    ...
    def _extract_shapes_from_node(self, node_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _call_einsum_generator(
        ...
        if node_type.startswith("conv"):
            if "Input" in shapes and "Weight" in shapes:
                ...
                return handler.generate_einsum_func(
                    shapes["Input"], shapes["Weight"], **conv_args
                )
            return None
    ...
    def _calculate_roofline_performance(
        self,
        compute_macs: int,
        memory_elements: int,
        ...
        """
        # Use normalized vendor-neutral throughput and bandwidth instead of
        # legacy vendor-specific per-cycle fields.
        ...
        normalized_precision = arch_config.get("precision_aliases", {}).get(
        ...
        # Utilization
        compute_util = (
            min(compute_cycles / total_cycles, 1.0) if total_cycles > 0 else 0
        )
        ...
        return {
            ...
            "operational_intensity": (
                compute_macs / memory_elements if memory_elements > 0 else 0
            ),
        }

    def save_analysis(self, analysis: AnalysisResult, output_path: str) -> None:
    ...
    def print_summary(self, analysis: AnalysisResult) -> None:
        ...
        print(
            f"Total Memory: {format_number(analysis.total['memory_elements'])} elements"
        )
        print(
            f"Operational Intensity: {analysis.total['operational_intensity']:.2f} MACs/element"
        )
